import { priorityLabels } from "../state/model.ts";
import type { PlannerSnapshot, Priority, Task, TaskDraft, TaskFilter } from "../state/model.ts";
import { formatPercent, pluralize } from "../utils/format.ts";
import { byId, setPressed, setText } from "./query.js";

export function renderDashboard(snapshot: PlannerSnapshot) {
  ...
  setText(
    "boardStatus",
    `${pluralize(snapshot.visibleTasks.length, "visible task")} from ${pluralize(
...
export function renderTaskList(snapshot: PlannerSnapshot, onToggle: (id: string) => void) {
  ...
  list.querySelectorAll<HTMLButtonElement>("[data-toggle-task]").forEach((button) => {
...
export function renderDraftPreview(draft: TaskDraft) {
  const preview = byId<HTMLOutputElement>("draftPreview");
  if (!preview) {
    return;
  }
...
export function renderPriorityButton(priority: Priority) {
  const button = byId<HTMLButtonElement>("priorityButton");
  if (button) {
    button.textContent = `Priority: ${priorityLabels[priority]}`;
  }
}

function updateFilterButtons(filter: TaskFilter) {
