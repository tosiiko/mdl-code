...
form@submit(handleLogin):
  ...
  .input@keydown(handleShortcut)

island@name(search-box)@mount(initSearch):
```

They compile to `data-mdl-on-*` markers. When a full document includes
configured scripts, MDL imports those modules and binds the named exported
functions. Browser event handlers receive the event. `mount` handlers run once
and receive the mounted element.

## Markdown

Markdown is rendered with `pulldown-cmark`. MDL only transforms section blocks and
`.name(...)` inline elements.
Raw HTML in Markdown is escaped, and unsafe Markdown link/image URL schemes are
neutralized before HTML output.
